...
docker buildx build --push --platform linux/amd64,linux/arm64 --no-cache -t svenwal/example-ticket-system:$tag .
echo "--- 2. Building current version as latest"
docker buildx build --push --platform linux/amd64,linux/arm64 -t svenwal/example-ticket-system:latest .
